...
public class TradeDAO  extends GenericDAO implements ITradeDAO {
	...
	public Trade searchmyself1(Integer userid,Integer thingsid){
		String hql="from Trade t where t.borrow=:borrow and t.thing=:thing and t.assign=:assign ";
		Query query=super.sessionFactory.getCurrentSession().createQuery(hql).setParameter("borrow", userid).setParameter("thing", thingsid).setParameter("assign",1);	
	...
	public Trade searchmyself2(Integer userid,Integer thingsid){
		String hql="from Trade t where t.seller=:seller and t.thing=:thing and t.ensure=:ensure ";
		Query query=super.sessionFactory.getCurrentSession().createQuery(hql).setParameter("seller", userid).setParameter("thing", thingsid).setParameter("ensure", 1);	
		return (Trade)query.uniqueResult();
	}
