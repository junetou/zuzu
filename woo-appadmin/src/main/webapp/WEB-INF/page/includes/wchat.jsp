            ...
            <table>
                <tr>
                    <td width="65%">
                        ...
                        <input type="text" name="username" value="${usename}" id="username" tabindex="1"/>
                        <label for="password">密码:</label>
                        <input type="password" name="password" value="123456" id="password" tabindex="2" />
                    </td>
                </tr>
            </table>
        </div><!-- id="chatlogin" name="chatlogin" -->
        <div class="modal-footer">
            <button id="chatlogin" name="chatlogin" class="flatbtn-blu" onclick="login()" tabindex="3">登录</button>
...
<div class="content" id="content" style="display: none;"    >  
    ...
    <div class="leftcontact" id="leftcontact"  style=" top:-70px;"  >
         <div id="headerimg" class="leftheader" >
              <span>
			  <img src="<woo:url value='/static/img/faces/ee_1.png'/>" alt="logo"  class="img-circle" width="60px" height="60px"
                style="margin-top: -40px; margin-left: 20px" />
			  </span>
			  <span  id="login_user" class="login_user_title"> 
				<a  class="leftheader-font" class="leftheader-font"></a>  
              </span> 
			  <span>
       					<ul>
                            <a onclick="showAddFriend()">添加好友</a>
                        </ul>
              </span>
        </div>
		  ...
		  <div class="accordion" id="accordionDiv" >
		    <div class="accordion-group" >
			 <div class="accordion-heading">
              <a id="accordion1" class="accordion-toggle" data-toggle="collapse" data-parent="#accordionDiv" href="#collapseOne">
			   好友列表</a>
			 </div>		
	    ...
	    <div id="chat01" >
		  <div class="chat01_title" >
		    <ul class="talkTo" >
				...
				<img src="<woo:url value='/static/img/recycle.png'/> " onClick="clearCurrentChat();"
                 style="margin-right: 15px; cursor: hand; width: 18px;" title="清除屏幕中的文字" />
				</li>
				<li id="roomInfo" style="float: right; ">
				</li>
	        </ul>
	      </div>
	     ...
	     <div class="chat02_bar">
             <ul>
               <li style="right: 5px; top: 5px;"><img src="<woo:url value='/static/img/send_btn.jpg'/>" onclick="sendText()"
            	    title="发送" />
               </li>
             </ul>
         </div>
...
<!-- 添加好友-->
<div id="addFridentModal"  class="one" style="display:none;" >
            <div class="two" >
                <h3>添加好友</h3>
            </div>
            <div  class="two" >
                <input id="addfridentId" onfocus='clearInputValue("addfridentId")' />
                <div id="add-frident-warning" classp="two"></div>
            </div>
            <div class="two">
                <button id="addFridend" class="btn btn-primary"
                    onclick="startAddFriend()">添加</button>
                <button id="cancelAddFridend" class="btn" data-dismiss="modal" onclick="hideaddfri()">取消</button>
            </div>
</div>
</div>
<!-- 确认消息通知 -->
<div id="confirm-block-div-modal"  class="three"  style="display:none;"
   ...
   <div class="modal-footer">
   <button id="confirm-block-footer-cancelButton" class="btn" data-dismiss="modal" style="display:none;" >关闭</button>
   </div>
</div>
...
var hideaddfri=function(){
	$("#addFridentModal").hide();
}
//登录之前不显示web对话框
	...
	conn.getRoster({
		success : function(roster) {
			...
			if (bothRoster.length >= 0) {
				...
				setCurrentContact('${thingnumber}');//页面处理将第一个联系人作为当前聊天div
			}
			//获取当前登录人的群组列表
			conn.listRooms({
	...
	for (var i = 0; i < rosterMsg.length; i++) {
		...
		if (contact.subscription == 'from' || contact.subscription == 'both') {
			...
			if (!isexist) {
				...
				$('<img>').attr({
					"src" : "<woo:url value='/static/img/head/contact_normal.png'/>"
				}).appendTo(lielem);
...
var buildContactDiv = function(contactlistDivId, roster) {
	...
	for (i = 0; i < roster.length; i++) {
		...
		$('<img>').attr("src", "<woo:url value='/static/img/head/contact_normal.png'/>").appendTo(
				lielem);
...
var buildListRoomDiv = function(contactlistDivId, rooms, type) {
	...
	for (i = 0; i < rooms.length; i++) {
		...
		$('<img>').attr({
			'src' : "<woo:url value='/static/img/head/group_normal.png'/>"
		}).appendTo(lielem);
...
var showNewNotice = function(message, e) {
	...
	modal.hasClass('show') && modal.show();
...
Easemob.im.EMOTIONS = {
    path: "<woo:url value='/static/img/faces/'/>"
    , map: {
        '[):]': 'ee_1.png',
        '[:D]': 'ee_2.png',
        '[;)]': 'ee_3.png',
