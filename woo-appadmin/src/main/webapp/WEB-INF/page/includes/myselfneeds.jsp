...
<tr>
	...
	<c:forEach items="${grid.datas}" var="data" varStatus="index" >
		...
		<woo:permission operationType="THINGS_VIEW" roleType="ROLE_THINGS">
									<a href="<c:url value='/portal/needs/detailmessages/${data.thingsId}'/>">
									[查看信息]
									</a>
		</woo:permission>
		<woo:permission operationType="THINGS_EDIT" roleType="ROLE_THINGS">
						      		<a href="javascript:void(0);" data-id="${data.thingsId }" class="btnEdit" data-url="needs/updatemyselfthings">
									[修改]
									</a>
								</woo:permission>
		<woo:permission operationType="THINGS_DELETE" roleType="ROLE_THINGS">
									<a href="javascript:void(0);"  class="btnDel" data-url="needs/deletethings/${data.thingsId }">
									[删除]
									</a>
		                        </woo:permission>
		</th>
